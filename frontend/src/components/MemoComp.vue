  ...
  <div class="memo">
    ...
    <ul>
      <li v-for="data in state.severData" :key="data.id" @click="edit(data.id)">{{ data.content }}</li>
        <!-- 객체들이 들어있는 배열(data1)에서 객체 하나를 꺼내서 data로 받아서 사용 -->
...
export default {
  setup() {
    ...
    const add=()=>{     
      ...
      axios.post("api/memos", {contentMessage:state.inputData}).then((res)=>{  
    ...
    const content=prompt("수정할 내용을 입력하세요", state.severData[id]);
    console.log("수정한 내용:", content);

      if(content!=null){  //content가 null이면 아래는 수행하지 않음.(수정한게 아무것도 없으면)
        axios.put("/api/edit", {indexData:id, contentMessage:content}).then((res)=>{  //객체하나에 2개의 데이터를 전송
